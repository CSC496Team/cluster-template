...
for i in range(6):
  ...
  node.addService(pg.Execute(shell="sh", command="sudo -H -u CSC496Team bash -c '/local/repository/ssh_setup.sh'"))
 
  node.addService(pg.Execute(shell="sh", command="sudo su CSC496Team -c 'cp /local/repository/source/* /users/CSC496Team'"))
